...
![Start with the decision, not a blank survey](docs/readme/01-start-with-decision.jpg)
...
![Frame the business decision](docs/readme/02-frame-the-study.jpg)
...
![Run customer conversations in parallel](docs/readme/03-run-fieldwork.jpg)
...
![Review every customer voice](docs/readme/04-review-evidence.jpg)
...
![See the signal in the evidence](docs/readme/05-see-the-signal.jpg)
...
![Move from evidence to a decision](docs/readme/06-decision-ready.jpg)
...
```mermaid
flowchart LR
    U[Your team] --> D[Research Director]
    D --> K[Knowledge Agent]
    D --> S[Study Designer]
    D --> F[Fieldwork Agent]
    D --> I[Interview Agents]
    D --> A[Analysis Agent]
    D --> R[Report Agent]
    K -. company context .-> S
    S -. approved study .-> F
    F -. consented participants .-> I
    I -. customer evidence .-> A
    A -. traceable findings .-> R
    R --> O[Decision-ready output]
```
